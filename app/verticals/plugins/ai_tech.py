"""Deterministic AI/Tech vertical plugin."""
...
class AITechVerticalPlugin:
    vertical_key = "ai_tech"
    ...
    def resolve_mode(self, *, profile) -> VerticalMode:
        raw = (getattr(profile, "preference_overrides", {}) or {}).get("verticals.ai_tech.mode")
    ...
    def _collect(self, *, profile) -> list[VerticalEvent]:
        ...
        events, health = collect_ai_tech_events(settings=settings, watchlist=getattr(profile, "all_watchlist_tickers", []))
    ...
    def breaking_candidates(self, *, profile, events: list[NormalisedEvent]) -> list[Any]:
        _ = events
        return [e for e in self._collect(profile=profile) if float((e.diagnostics or {}).get("deterministic_score", 0.0)) >= 0.82][:3]
    ...
    def audit_metrics(self, *, profile, session_key: str, candidate_events: list[NormalisedEvent] | None = None) -> dict[str, Any]:
        ...
        source_status = self._last_source_health or {
            "sec": {"status": "disabled" if mode == "off" else "unavailable"},
            "arxiv": {"status": "stub_inactive"},
            "github": {"status": "stub_inactive"},
        }
        return {
            ...
            "watchlist_exposure_summary": f"ai_event_count={len(events)}",
