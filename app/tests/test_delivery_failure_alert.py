...
class TestSendDeliveryFailureAlert:
    def _mock_db_session(self, recent_alert=None):
        """Return a (patch_target_side_effect, mock_sess) pair.

        get_session is called twice in _send_delivery_failure_alert: once for
        the cooldown query and once to save the SentMessage. We build a factory
        that returns a fresh context-manager mock on each call so both uses work.
        """
        mock_sess = MagicMock()
        mock_sess.query.return_value.filter.return_value.first.return_value = recent_alert
    ...
    def test_cooldown_suppresses_duplicate_alert(self):
        make_cm, mock_sess = self._mock_db_session(recent_alert=MagicMock())
        kwargs = _alert_kwargs()
        with patch("app.main.get_session", side_effect=make_cm), \
             patch("app.main.TelegramMessenger") as mock_tg:
            _send_delivery_failure_alert(**kwargs)
        mock_tg.assert_not_called()

    def test_no_cooldown_match_sends_alert(self):
        make_cm, mock_sess = self._mock_db_session(recent_alert=None)
