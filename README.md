...
- Session-based briefing system (Europe/Madrid): six canonical sessions, all using the shared premium renderer

| Session key | Label | Focus | Window |
|---|---|---|---|
| `morning` | Morning Briefing | Asia overnight + Europe open + US prior close | 06:00-10:30 |
| `europe_midday` | Europe Midday Check | Europe session | 10:30-13:30 |
| `us_pre_open` | US Pre-Open Setup | US setup | 13:30-15:30 |
| `us_intraday_risk` | US Intraday Risk Check | US session | 15:30-17:30 |
| `into_close` | Into Close Update | Late US session | 17:30-22:00 |
| `closing_wrap` | Closing Wrap / Next-Day Setup | US close + next-day setup | 22:00-23:59 |

Asia coverage: Asia is covered inside Morning Briefing (Asia overnight + Europe open + US prior close). A dedicated Asia-open session can be added later if needed.
...
- **Phase 9.8 (exact snapshot replay):** `python -m app.cli snapshots replay --date <date> --session <key>` re-delivers a stored session snapshot through the live messenger channels without making any provider or LLM calls. Content is exactly what was captured at archive time. A blue SNAPSHOT REPLAY banner is prepended to Telegram text, email plain text, and email HTML so recipients can distinguish replays from live sends; `--no-banner` suppresses it. `--channel telegram|email|all` routes to one or both channels. No idempotency keys are written and no new snapshot is created. Missing content in the snapshot (e.g. email HTML was not stored) is reported as a skip with a reason, not an error. Delivery failures populate `result.errors` and exit with status 1. Implemented in `app/briefing/snapshot_replay.py`; 17 tests across banner content, channel routing, missing content, and failure capture.
...
- **Phase 9.9 (reliability hardening and session rendering audit):** eliminates the duplicate-send race condition caused by two separate polling jobs (`morning_cadence_check` + `intraday_cadence_check`) that both called `run_session_brief()`. Replaced with a single `session_cadence_check` job (`max_instances=1`) covering all six session windows. New `app/briefing/session_metadata.py` is the canonical source of truth for session labels, focus lines, and windows. Channel routing fixed: all six sessions now resolve their delivery channels via the session key directly (falling back to morning config), so email is consistently included for all sessions. New `run_daily_summary()` function and `daily-summary` CLI command prints per-session send state, focus line, archived snapshots, and breaking alert count — no provider calls. `schedule-status` upgraded to show session label, focus, and window for current and next sessions, plus a full session schedule table and a warning if the scheduler is running alongside a manual CLI invocation. Signal quality: energy/blockade relevance templates now require a genuine energy-core keyword (`opec`, `oil`, `crude`, etc.) or a shipping-and-geopolitical combination — prevents misclassification of non-energy stories. Clickbait filter extended to suppress "hiding in plain sight", "at a discount", "screaming buy", and similar opinion/valuation-commentary patterns from sector scan. Asia coverage note: Asia is covered inside Morning Briefing (Asia overnight + Europe open + US prior close); a dedicated Asia-open session can be added later. 32 new tests added (scheduler, duplicate-send guard, session metadata, daily summary). Troubleshooting note: do not run launchd, Docker, systemd, and a manual scheduler simultaneously — the file-lock prevents a second scheduler on the same machine, but an admin running `python -m app.cli brief` alongside an active scheduler can still trigger a concurrent send attempt during the idempotency window.
- **Phase 9.X (MoveContextEngine):** deterministic per-instrument move, range, and level context for all Telegram and email outputs. `app/briefing/move_context.py` answers four questions for every market number: direction (up/down/flat), move magnitude relative to normal daily moves (static thresholds or 1Y percentile when history is available), current level vs 1Y range (near highs/lows or mid-range), and where the current print sits within today's intraday range (near session highs/lows/mid-range/upper/lower half). Treasury yields are always shown in basis points, never as a percentage change of the yield level. VIX carries a regime label (Calm/Watchful/Stressed/Panic) instead of a magnitude label. Session-mode-aware range labels (e.g. "pre-market range", "full session range", "prior session range") adapt to the session context. `format_session_change()` generates plain-English session-to-session "what changed" lines, suppressing meaningless zero-change outputs. Integrated into `TelegramFormatter._format_market_setup()` (index quotes, macro quotes, Treasury yields) and `_format_watchlist()` (compact per-instrument move labels with context). 101 tests covering all label thresholds, spec cases (Russell 2000 Strong day/near session highs, US 10Y -3 bp Mild rally, US 2Y +7 bp Firm selloff, WTI Sharp fall/high vs 1Y range, Gold upper half, VIX Watchful/falling), flat-move rendering, missing-history fallback, and formatter output.
